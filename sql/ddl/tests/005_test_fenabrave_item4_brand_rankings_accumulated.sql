-- 005_test_fenabrave_item4_brand_rankings_accumulated.sql

-- Validacao estrutural do item 4 da fase 2 Fenabrave.
SELECT
  source_file_id,
  reference_period,
  vehicle_category,
...
WHERE item_code = 'fenabrave_item_04_ranking_por_marca_acumulado'
GROUP BY source_file_id, reference_period, vehicle_category
ORDER BY reference_period, vehicle_category;

-- Cobertura mensal do item 4 por PDF Fenabrave.
SELECT
  f.reference_period,
...
LEFT JOIN public.market_fenabrave_extraction_items i
 ...
 AND i.item_code = 'fenabrave_item_04_ranking_por_marca_acumulado'
