...
select
  r.source_file_id,
  r.reference_period,
  extract(year from r.reference_period)::int as reference_year,
  extract(month from r.reference_period)::int as reference_month,
  to_char(r.reference_period, 'YYYY-MM') as reference_month_label,
  i.item_code,
  i.item_label,
  i.pdf_page,
...
source_files as (
  select
    ...
    f.reference_period,
    extract(year from f.reference_period)::int as reference_year,
    extract(month from f.reference_period)::int as reference_month,
    to_char(f.reference_period, 'YYYY-MM') as reference_month_label,
    s.source_name,
    f.source_url,
    f.source_page_url,
    f.storage_bucket,
