...
- [x] Padronizar exclusao de `dead posts` das metricas e views analiticas. Fechado em 2026-08-04 com exclusao por `NOT EXISTS`/`active_posts` nas views analiticas, RPC `confirm_unavailable_posts(...)` e validacao estrutural do contrato SQL. Os posts confirmados como `unavailable` permanecem apenas em auditoria e revisao humana.
