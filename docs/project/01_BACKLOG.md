...
- [x] Validar a primeira carga completa da Fenabrave apos a implementacao de todas as paginas ativas do PDF e fechar os pontos de carga/validacao de dados no backlog. Fechado em 2026-08-10 com a validacao real do `reference_period = 2026-07-01` (`market_source_files.id = 31`): arquivo canonico unico, fase 1 consistente, itens ativos `1..8` e `11..22` com `status = validated` e `validation_status = passed`, itens graficos `13..18` aprovados no estado final persistido e evidencia registrada em `docs/external_data/65_FENABRAVE_PRIMEIRA_CARGA_COMPLETA_VALIDACAO_2026_07.md`.
...
- [x] Isolar e estabilizar o incidente de `Segmentation fault` do Streamlit Cloud. Fechado em 2026-07-15 apos reintroducao controlada de `st.plotly_chart` e `st.dataframe` em `Criador individual`, `Data Quality`, `Cadastro de Criadores` e Fenabrave, com troca de componentes pesados para `width="stretch"` e remocao do cache do cliente Supabase.
- [x] Reintroduzir os graficos interativos do criador individual de forma controlada. Fechado em 2026-07-15: os graficos Plotly, a tabela de videos e a tabela tecnica foram religados e testados sem nova queda reportada.
